...
class AE(torch.nn.Module):
    def __init__(self, input_size=None, sigma=.0, latent_size=64, rnn_layers=64, teacher_forcing_ratio=1.):
    ...
    def forward(self, X): 
        init_h = self.encoder_init_hidden(X.size(0))
        lstm_out, h_n = self.encoder(X, init_h)
